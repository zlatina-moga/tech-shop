...
const Gaming = () => {
  ...
  const [baseLink, setBaseLink] = useState("/accesorii/gaming-console");
  ...
  useEffect(() => {
    ...
    if (brand) {
      ...
      productService.getAllGamingByBrand(currentPage, brand).then((result) => {
        ...
        setBaseLink(`/accesorii/gaming-console?brand=${brand}`);
      });
      sortingService.getHighestPriceByBrand(91, brand).then((response) => {
        setHighestPrice(response[1]);
      });
    } else {
      setShow(false);
      productService
        ...
        .then((result) => {
          ...
          setShow(true);
          setTotalPages(result[0].totalPages);
          setBaseLink(`/accesorii/gaming-console`);
        })
        .catch((err) => {
          console.log(err);
        });
  ...
  useEffect(() => {
    if (priceRange != '' && brand && selectedSort != "/accesorii/gaming-console") {
      setShow(false);
      const sort = selectedSort.split("=")[2];
      productService
        .getSortedGamingPriceAndBrand(priceRange, currentPage, sort, brand)
        .then((result) => {
          setLaptopsData(result);
          setShow(true);
          setTotalPages(result[0].totalPages);
    ...
    } else if (brand && selectedSort != "/accesorii/gaming-console") {
      setShow(false);
      const sort = selectedSort.split("=")[1];
      productService
        .getSortedGamingBrand(currentPage, sort, brand)
        .then((result) => {
          setLaptopsData(result);
          setShow(true);
          setTotalPages(result[0].totalPages);
    ...
    } else if (priceRange != '' && !brand && selectedSort != "/accesorii/gaming-console") {
      const sort = selectedSort.split("=")[1];
      productService
    ...
    } else if (selectedSort != "/accesorii/gaming-console") {
  ...
  return (
    <>
      ...
      {loading ? (
        ...
        <>
          <LaptopsPage
            title={`Accesorii Gaming & Console ${pageTitle}`}
            ...
            baseLink={baseLink}
            brands={brands}
            brandLink={"/accesorii/gaming-console?brand="}
