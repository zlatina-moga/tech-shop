...
const MonitoareTouchSH = () => {
  ...
  const [selectedSort, setSelectedSort] = useState(
    "/monitoare/touchscreen-refurbished"
  );
  ...
  useEffect(() => {
    productService
      .getAllTSRefMonitors(currentPage)
      .then((result) => {
        setLoading(false);
  ...
  useEffect(() => {
    if (priceRange) {
      ...
      productService
        .geSortedTSRefMonitorsPrice(priceRange, currentPage, sort)
        .then((result) => {
          setLaptopsData(result);
        })
        .catch((err) => {
      ...
      const sort = selectedSort.split("=")[1];
      productService
        .geSortedTSRefMonitors(currentPage, sort)
        .then((result) => {
          setLoading(false);
          setLaptopsData(result);
        })
  ...
  useEffect(() => {
    setShow(false);
    productService
      .getAllTSRefMonitorsPrice(priceRange, currentPage)
      .then((result) => {
        setLaptopsData(result);
        setShow(true);
      })
...
export default MonitoareTouchSH;
