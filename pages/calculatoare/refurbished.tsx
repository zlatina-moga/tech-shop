...
const Calculatoare = () => {
  ...
  const [selectedSort, setSelectedSort] = useState("/calculatoare/refurbished");
  ...
  useEffect(() => {
    if (priceRange) {
      ...
      productService
        .getSortedRefurbishedComputersPrice(priceRange, currentPage, sort)
        .then((result) => {
          setLaptopsData(result);
        })
        .catch((err) => {
      ...
      const sort = selectedSort.split("=")[1];
      productService
        .getSortedRefurbishedComputers(currentPage, sort)
        .then((result) => {
          setLoading(false);
          setLaptopsData(result);
        })
  ...
  useEffect(() => {
    setShow(false);
    productService
      .getAllRefurbishedComputersPrice(priceRange, currentPage)
      .then((result) => {
        setLaptopsData(result);
        setShow(true);
      })
